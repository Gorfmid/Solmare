# ACT IV — CHAPTERS 17–24 GENERATION INSTRUCTIONS

## FIRST DOCTRINE

**Scope:** Chapters 17–24 (Act IV)  
**Status:** Structural shells — prose pending  
**Begins:** Immediately after Chapter 16 — border intelligence complete; strategic reckoning begins  
**Ends:** Chapter 24 — Book One fully resolved

**Prior file:** `act4_chapters_19-24_generation.md` — **superseded** (legacy anomaly/Fold resolution arc removed)

**Related documents:**
- `chapter_list.md` — plot roadmap
- `act3_chapters_9-16_generation.md` — Act III precedent
- `../State/current_state.md`
- `../../../Writing_System/Series_Principles/series_principles.md`
- `../../../Writing_System/Style_Guide/the_70_30_rule.md`

---

## Act IV Function

**Fleet doctrine shift and first strategic understanding** of the structured opposing force.

- Not cosmological mystery payoff
- Not reality-fracture resolution
- **Strategic assessment** — what Fleet will believe, how it will act, what changes permanently

Act IV emphasizes **humanity and institution** under irreversible knowledge — Rowan, Sato, Mercer, Holt/Nand chain, Calder as witness.

---

## Book One Completion Requirement

Book One **must conclude** in Ch. 24. Sequel hook from **consequences of doctrine shift**, not withheld mystery.

By Ch. 24:

- Structured opposing force **acknowledged** at strategic level (operational truth — not full cosmology)
- *Veil* crew mission **complete** for Book One arc
- Calder's Book One arc **complete**
- Fleet Authority **permanently changed** in how it reads the border

---

## Chapter Roadmap

### Chapter 17 — The Listening Post

Fleet and Cartography nodes as strategic instruments; Holt/Nand elevated in comms chain.

### Chapter 18 — Rules of Engagement

Formal ROE debate for structured unknown force — doctrine, not improvisation.

### Chapter 19 — First Witness

*Veil* testimony enters strategic record; Calder as witness.

### Chapter 20 — The Weight of Evidence

Competing Fleet interpretations; political friction over opposing presence.

### Chapter 21 — The Long Shadow

Strategic consequences; Collectives thread intersects border intelligence.

### Chapter 22 — Recognition

Fleet acknowledges structured opposing force — name/class/doctrine without full enemy resolution.

### Chapter 23 — The Last Report

Final intelligence synthesis; Maris-thread echo; actionable strategic record.

### Chapter 24 — The Veil Opens

Book One resolution — doctrine shift complete; *Veil Opens* as strategic opening, not anomaly reveal.

---

## Core Philosophy

Do not answer: *"What is the anomaly?"*

Answer: *"What does Fleet now know — and what will it do?"*

The opposing presence becomes **strategically understandable**. It must not become **ordinary**.

---

## The 70/30 Rule

**~85%:** crew, Fleet politics, doctrine debate, testimony, institutional pressure  
**~15%:** residual sensor ambiguity from border operations

---

## Generation Checklist (Per Chapter)

- [ ] Strategic accumulation — not episodic revelation
- [ ] Holt/Nand / Fleet Command continuity from Incident arc
- [ ] No Fold / Library / living map legacy
- [ ] Resolution boundaries per `series_principles.md`
- [ ] Ch. 24 delivers complete Book One arc

---

## Act IV Final Checklist

- [ ] Doctrine shift legible to reader
- [ ] Incident (Ch. 5–8) pays forward in strategic record
- [ ] Maris buried-warning thread acknowledged where appropriate
- [ ] Sequel problem arises from **resolution**, not holes
