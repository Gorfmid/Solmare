# Book One — The Kestrel Veil Incident

**Series:** The Solmare Cycle  
**Status:** Ch. 1–24 + Epilogue **LOCKED FINAL** · **ACT IV COMPLETE** · **END BOOK ONE** · **4-act architecture**

See `Notes/book1_core_premise.md` and `Notes/REWRITE_STATUS.md`.

## 4-Act Structure

| Act | Chapters | Arc |
|-----|----------|-----|
| I — Routine Patrol | 1–4 | ✓ **LOCKED FINAL** |
| II — The Kestrel Veil Incident | 5–8 | ✓ **LOCKED FINAL** |
| III — Shadows Beyond the Border | 9–16 | ✓ **LOCKED FINAL** |
| IV — First Doctrine | 17–24 | ✓ **LOCKED FINAL** |

## Folders

| Folder | Contents |
|--------|----------|
| `Chapters/` | Chapter prose (`chapter_1.md` … `chapter_24.md`) |
| `Manuscript/` | Master Markdown, build script |
| `Act_Outlines/` | 4-act generation directives, `chapter_list.md` |
| `State/` | Runtime state (`current_state.md`, chapter logs) |
| `Notes/` | Premise, outline, narrative spine, rewrite status |
| `Cover/` | Cover placeholder |

## Shared canon

Character sheets, universe rules, and writing system docs live at repository root (`Characters/`, `Universe/`, `Writing_System/`).
