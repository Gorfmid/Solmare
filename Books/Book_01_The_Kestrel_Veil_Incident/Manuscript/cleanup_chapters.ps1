# Strip author metadata; apply publication chapter headers. Preserves UTF-8.
$ErrorActionPreference = "Stop"
$chaptersDir = Join-Path (Split-Path $PSScriptRoot -Parent) "Chapters"
...
$titles = @{
    ...
    7  = "Ghost Fleet Contact"
    8  = "The Empty Vessel"
    9  = "The Silent War"
    10 = "Running Dark"
    11 = "First Doctrine Contact"
    12 = "Broken Maps"
    13 = "Espionage"
    14 = "Lines of Loyalty"
    15 = "Siege of Korrath"
    ...
    19 = "First Truth"
    20 = "Fleet Divided"
    21 = "Last Transmission"
    22 = "Battle for Reality"
    23 = "Dead Reckoning"
    24 = "The Solmare Quadrant"
}

function Test-MetadataLine([string]$Line) {
    return $Line -match '^\*\*(Chapter Number|Title|POV Character|Location|Ship Class|Word Count):\*\*'
}

function Get-SectionNumber([string]$Line) {
    if ($Line -match '^## SECTION (\d+)\s*[\u2014\-–-]\s*.+$') { return [int]$Matches[1] }
    return $null
}

function Test-SceneLine([string]$Line) { return $Line -match '^## Scene \d+\s*[\u2014\-–-]' }
function Test-EventLine([string]$Line) { return $Line -match '^\*\*Event\s+\d+' }
function Test-CodaLine([string]$Line) { return $Line -match '^## (CODA|EPILOGUE)\s*[\u2014\-–-]' }
...
function Get-CleanedBody {
    param([string[]]$Lines, [int]$ChapterNum)
    ...
    for ($i = $start; $i -lt $Lines.Count; $i++) {
        ...
        if ($skipMode -eq 'section_2') {
            ...
            if (Test-Section2NarrativeStart $line) {
                $skipMode = $null
                $out.Add($line)
                continue
            }
            if (Test-Section2ObjectiveLine $line) { continue }
            # Unlabeled objective prose (mission statement paragraphs)
            continue
        }
...
foreach ($n in 1..24) {
    ...
    if ($raw.Count -gt 0 -and $raw[-1] -eq '') { $raw = $raw[0..($raw.Count - 2)] }

    $body = Get-CleanedBody -Lines $raw -ChapterNum $n
    $header = "# Chapter $n`n`n## $($titles[$n])`n`n$([char]0x2766)`n"
    if ($body.Count -eq 0) {
        $text = $header + "`n"
    } else {
        $text = $header + "`n" + ($body -join "`n").TrimEnd() + "`n"
    }
    [System.IO.File]::WriteAllText($path, $text, $utf8)
    $words = ($text | Measure-Object -Word).Words
    Write-Host "cleaned chapter_$n.md ($words words)"
}
